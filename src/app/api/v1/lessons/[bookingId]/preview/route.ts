...
import { previewLessonMessage } from "@/infrastructure/lessons/lesson-operations";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ bookingId: string }> }) {
  ...
  const parsed = z.object({ kind: z.enum(["recap", "follow_up"]), correction: z.string().max(5000).optional() }).strict().safeParse(await request.json().catch(() => null));
  if (!parsed.success) return problem(400, "invalid_request", "Check the message");
  const result = await previewLessonMessage(auth.session.workspaceId, bookingId, parsed.data);
  if (!result.ok) return problem(result.reason === "not_found" ? 404 : 409, result.reason, "This message cannot be previewed yet");
  return NextResponse.json({ data: result.value }, { headers: { "Cache-Control": "private, no-store" } });
}
