...
1. **Domain and persistence:** define a lesson identity/read model that resolves a confirmed booking in its workspace; add Drizzle tables/migration for editable debrief draft, optional per-lesson skill assessments, immutable sent recap/follow-up, and a durable email outbox or equivalent delivery record. Separate private and shared columns/entities. Avoid cascading deletion of sent history through an editable draft. Plan the stable learner key using the current contact/email model; do not invent learner accounts.
...
- [x] Domain and persistence code: `src/domain/lessons/`, `src/infrastructure/database/schema.ts`, migrations `drizzle/0009_graceful_tempest.sql` and `drizzle/0010_puzzling_talon.sql`. A debrief belongs to exactly one collection or legacy booking; skills and immutable-message rows are separate. **Migrations have only been generated, not applied.**
...
- [x] Local static verification: `npm run verify` passes (CSS/style guards, typecheck, 41 tests, lint, production build). This does **not** include a migrated-database walkthrough, mobile/keyboard inspection, or provider delivery test.
- [ ] Apply migrations to the intended environment only after confirming which Neon database/staging branch is targeted; then run the acceptance walkthrough with real bookings.
...
- Never commit credentials or apply a production migration without explicit scope/verification.
