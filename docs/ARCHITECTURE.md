...
In local development only, `DEV_WORKSPACE_ID` resolves a seeded workspace through the same database-backed routes, without a cookie or trial gate. It is disabled outside `NODE_ENV=development`. In normal operation, the instructor requests a six-digit email code sent by the branded provider, verifies it within ten minutes, and signs into Firebase with a short-lived custom token. Google sign-in also uses Firebase. The server verifies the Firebase ID token before issuing an opaque, revocable session cookie; its hash resolves to an active instructor and workspace in Neon. First-time instructors enter their full name on a separate onboarding page. Pilot workspaces remain writable until explicitly ended; the post-pilot trial and paid entitlements retain read-only expiry rules. Public booking remains protected by purpose-scoped links. Notes and debrief endpoints do not exist yet; their isolated components in `/component-lab` are examples, not working features.
