...
19. The development database lives in a separate Neon account from the user's existing CLI account. Its credentials and the isolated CLI login remain git-ignored. Calendar availability is the first live vertical slice: create, list, and persist across refresh. Booking, Today, release, and debrief workflows follow as separate slices.
